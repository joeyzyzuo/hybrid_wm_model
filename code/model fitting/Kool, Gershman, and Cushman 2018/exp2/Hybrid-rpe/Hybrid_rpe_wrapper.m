...
function results = Hybrid_rpe_wrapper
...
% data = groupdata.subdata(1);

% run optimization
f = @(x,data) Hybrid_rpe_llik(x,data);
results = mfit_optimize(f,set_Hybrid_rpe,data,nstarts);
...
writetable(t, ['param_Hybrid_rpe.csv'], 'WriteRowNames',true,'Delimiter',';');
