...
function bolenz_modelfitting_Hybrid_lwm
...
% get all elements in logfile folder
files = dir('./logfiles');
...
for fidx = 1:numel(files)
    ...
    % import logfile
    data = importdata(['./logfiles/',char(fname)]);
...
opts.fai = 0; % indexes presence of response stickiness
opts.st = 1; % indexes presence of response stickiness
...
save('./results_bolenz_Hybrid_lwm.mat', 'results');
...
for m = 1:nrmodels
    ...
    writetable(t, ['./params_bolenz_Hybrid_lwm.csv'], 'WriteRowNames',true,'Delimiter',';');
end
end