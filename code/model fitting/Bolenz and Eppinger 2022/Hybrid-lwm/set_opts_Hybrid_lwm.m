...
function [opts, param] = set_opts_Hybrid_lwm(opts)
...
param(6).name = 'mixing weight low gain';
param(6).logpdf = @(x) sum(log(betapdf(x,2,2)));
param(6).lb = 0;
param(6).ub = 1;

param(7).name = 'mixing weight high gain';
param(7).logpdf = @(x) sum(log(betapdf(x,2,2)));
param(7).lb = 0;
param(7).ub = 1;

param(8).name = 'mixing weight low loss';
param(8).logpdf = @(x) sum(log(betapdf(x,2,2)));
param(8).lb = 0;
param(8).ub = 1;

param(9).name = 'mixing weight high loss';
