...
function  [LL, dtQ] = Hybrid_lwm_rllik(x,subdata,opts)
...
eta_variable = 1;        % transition learning rate
kappa_variable = 1;      % counterfactual transition learning rate
w_WM_low_gain = y(6);        % mixing weight RL_low_stake
w_WM_high_gain = y(7);       % mixing weight RL_high_stake
w_WM_low_loss = y(8);      % mixing weight WM_stable
w_WM_high_loss = y(9);     % mixing weight WM_variable
...
for t = 1:N 
    ...
    if subdata.stake(t) == 1 && subdata.blockCondition(t) == 1
        wwm = w_WM_low_gain;
        eta = 1;
        kappa = 1;
    elseif subdata.stake(t) == 5 && subdata.blockCondition(t) == 1
        ...
        eta = 1;
        kappa = 1;
    elseif subdata.stake(t) == 1 && subdata.blockCondition(t) == 0
        wwm = w_WM_low_loss;
        eta = 1;
        kappa = 1;
    ...
    dtQ(2) = point_sign - Q2(s2);                            % prediction error (2nd choice)
    ...
    % wm decay
    if subdata.blockCondition(t) == 1 %gain
        regression_mean = 4.5;
    else
        regression_mean = -4.5;
    end
    ...
    if s2==1
        W2(2) = W2(2) + decay_fai * (regression_mean-W2(2)); 
    else
        W2(1) = W2(1) + decay_fai * (regression_mean-W2(1));
    end           
end
end