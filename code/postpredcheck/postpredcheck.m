...
% create output file
fileID = fopen('./trialdata_Hybrid_wm_sim.csv' ,'w');
% write column names in output file
fprintf(fileID, 'id;simrun;version;trial;transitions;stake;s1;choice;s2;points;reward1;reward2\n');

% load modeling results
results = importdata("./results_Hybrid_wm.mat");
x = results.x;
subID = results.subID;

% load trialdata in order to retrieve task versions
[~,txt,~] = xlsread('./trialdata_zuo.csv');

% add counterfactual transition learning rate (equal to transition learning
% rate)
x = [x(:,1:4), x(:,4), x(:,5:end)];

nruns = 50; % how often to simulate data for each subject
size_txt=size(txt);
