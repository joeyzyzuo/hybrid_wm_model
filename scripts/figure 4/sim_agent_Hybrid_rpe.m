...
function  output = sim_agent_Hybrid_rpe(x,env)
...
w_low_stable = x(6);           % mixing weight low stakes no switches
w_high_stable = x(7);          % mixing weight high stakes no switches
w_low_variable = x(8);             % mixing weight low stakes with switches
w_high_variable = x(9);            % mixing weight high stakes with switches
st = x(10);
respst = x(11);
